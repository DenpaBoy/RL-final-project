...
class Agent:
    def __init__ (self,world):
        ...
        self.hidden_width1 = 200
        self.hidden_width2 = 200
        self.world = world
        ...
        with tf.variable_scope("action_scorer"):
            with tf.variable_scope("Linear1"):
                W1 = tf.get_variable("Weight",shape = [self.vec_dim, self.hidden_width1],initializer = initializer)
                b1 = tf.get_variable("Bias",shape = [self.hidden_width1],initializer = initializer)
                h1 = tf.matmul(v_s,W1) + b1
            with tf.variable_scope("Relu2"):
                W2 = tf.get_variable("Weight",shape = [self.hidden_width1, self.hidden_width2],initializer = initializer)
                b2 = tf.get_variable("Bias",shape = [self.hidden_width2],initializer = initializer)
                h2 = tf.nn.relu(tf.matmul(h1,W2) + b2)
            with tf.variable_scope("Linear3a"):
                W3a = tf.get_variable("Weight",shape = [self.hidden_width2//2, self.action_num],initializer = initializer)
