...
with tf.Session() as sess:
     ...
     for e in range(episodes):
         ...
         quest_id = np.random.randint(5)
         home_env.new_game()
         home_env.current_quest = home_env.quests[quest_id]
         ...
         if (e+1)%display_step == 0:
            print('Episode '+str(e+1)+' : Reward sum = '+str(reward_sum))
     ...
     for test in range(1,test_round+1):
         ...
         for t in range(1,T+1):
             ...
             if quest_str == home_env.quests[quest_id] and location_str == home_env.locations[quest_id] and a_select == 4+quest_id :
                success_num += 1
     ...
     plt.title('Random quest reward sum')          
     plt.plot(e_x,episodes_reward_sum,'b')             
     plt.savefig('Random_reward_sum.png') 


     f1 = open('Random_quest_success_rate.txt','w')  
     f1.write('Random_quest_success_rate = '+str(success_rate*100)+' %')
     f1.close()

     f2 = open('Random_quest_demo_data.txt','w') 
